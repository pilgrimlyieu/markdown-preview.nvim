...
function testMultiPortSupport () {
  ...
  assert.match(server, /plugin\.nvim\.call\('mkdp#rpc#preview_data', \[Number\(bufnr\), 1\]\)/)
  assert.match(server, /sendToClient\(client, 'refresh_content', initialData\)/)
  assert.doesNotMatch(server, /const buffers = await plugin\.nvim\.buffers/)
  assert.doesNotMatch(server, /buffer\.getLines\(\)/)
...
function testCursorSyncUsesLightweightEvent () {
  ...
  assert.match(rpc, /function! s:preview_state\(bufnr\)/)
  ...
  assert.match(rpc, /'isActive': bufnr\('%'\) ==# a:bufnr/)
...
function testDebouncedContentRefresh () {
  ...
  assert.match(rpc, /function! s:refresh_content_data\(bufnr\)/)
  assert.match(rpc, /function! mkdp#rpc#preview_data\(bufnr, include_content\)/)
  assert.match(rpc, /if !bufexists\(a:bufnr\)/)
  assert.match(rpc, /let l:data = s:preview_state\(a:bufnr\)/)
  assert.match(rpc, /let l:data\['changedtick'\] = getbufvar\(a:bufnr, 'changedtick'\)/)
  assert.match(rpc, /let l:data\['pageTitle'\] = get\(g:, 'mkdp_page_title', ''\)/)
  assert.match(rpc, /let l:data\['theme'\] = get\(g:, 'mkdp_theme', ''\)/)
  assert.match(rpc, /let l:data\['name'\] = bufname\(a:bufnr\)/)
  assert.match(rpc, /let l:data\['content'\] = getbufline\(a:bufnr, 1, '\$'\)/)
  assert.match(rpc, /function! s:send_refresh\(bufnr, \.\.\.\)/)
  assert.match(rpc, /'data': mkdp#rpc#preview_data\(a:bufnr, 0\)/)
  assert.match(rpc, /s:notify_server\(a:bufnr, 'refresh_content', s:refresh_content_data\(a:bufnr\)\)/)
...
function testFreshRefreshSkipsFullContent () {
  ...
  assert.ok(snapshotRead < changedtickRead, 'expected refresh snapshot before changedtick fallback')
  ...
  assert.match(server, /markContentFresh\(\{ bufnr, changedtick: initialData\.changedtick \}\)/)
