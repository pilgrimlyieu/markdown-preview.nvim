...
export default function umlPlugin(md: MarkdownIt, options: PlantumlOptions = {}) {
  ...
  function uml(state: StateBlock, startLine: number, endLine: number, silent: boolean) {
    let nextLine: number
    let markup: string
    let params: string
    let i: number
    ...
    const token = state.push('uml_diagram', 'div', 0)
    token.block = true
  ...
  md.renderer.rules.uml_diagram = function (tokens, idx) {
    ...
    const alt = token.info ? token.info.slice(1) : 'uml diagram'
